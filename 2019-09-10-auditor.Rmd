...
Machine learning is a hot topic nowadays, thus there is no need to convince anyone about its usefulness. ML models are successfully applied in biology, medicine, and finance. With modern software, it is easy to train even a complex model that fits the training data and results in high accuracy on the test set. The problem arises when poorly verified model fails confronted with real-world data.

In this post, we describe  [`auditor`](https://github.com/ModelOriented/auditor) package for visual auditing of **residuals** of machine learning models. 
...
However, this function can generate plots only for linear models and some of these plots are not extendable to other models. 
At the same time, many algorithms, such as random forests or neural networks are often treated as black boxes and there are few or no methods for the analysis of errors in these models. 
The auditor comes over these problems. It is a toolbox with validation plots that are model-agnostic, which means that they can be used regardless of the expected distribution of residuals. 
The auditor provides flexible and consistent grammar for validation of any model class.
...
Plots presented in this post are only a small part of the auditor's features. To explore more, for example other types of plots and their interactive versions, we encourage you to visit the following sources;
