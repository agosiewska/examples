...
`auditor` adds another component to the boxplot which is the root mean square error (RMSE) measure, shown as `✕`. For the appropriate model, most of the residuals should lay near zero %% to zdanie chyba nie tutaj albo inaczej, no bo nie blisko zera tylko ewentualnie wokół box'ów%%. A large spread of values indicates problems with a model. Comparing our two models we can see that random forest model is much more spreaded (worse) that linear one.
...
Plots presented in this post are only a small part of the auditor's capacities. Among other ploting functions focusing on residuals, there are also functions focusing on model preformance or model evaluation. What is more, most of the ploting functions are available in an interactive version (D3). We encourage you to visit the following sources:
